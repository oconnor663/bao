...
/// "header region") with a given hash. If the length of the region is less
/// than or equal to the chunk size, the region's hash is simply the hash of
/// that chunk. If it's longer than a chunk, then the the region is encoded as
/// a tree, and it's hash is the hash of the node at the top of that tree.
///
...
///
/// Note that a complete encoded file is both the encoded "header region", and
